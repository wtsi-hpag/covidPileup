...
{
    FILE *namef;
    int i,j,nSeq,args,num_hits;
    int n,*hit_index;
    int n_contig,n_reads,n_readsMaxctg,nseq;
    fasta *seq;
    void Memory_Allocate(int arr);
    void ArraySort_String(int n,char **Pair_Name,int *brr);
    char line[2000]={0},tempc1[60],cc[60],RC[2],rdname[60],*st,*ed;
    char **cmatrix(long nrl,long nrh,long ncl,long nch);

    seq=NULL;
    if(argc < 2)
    {
      printf("Usage: %s <input_covid_genome_names> <output_country_names>\n",argv[0]);
...
/*  read the alignment files         */
    i=0;
    while(fscanf(namef,"%s",rdname)!=EOF)
    {
        st = rdname;
        ed = strchr(rdname,'/');
...
//    ArraySort_String(nseq,R_Name,hit_index);
    ...
    printf("Job finished for %d countries!\n",num_hits);
    return EXIT_SUCCESS;
